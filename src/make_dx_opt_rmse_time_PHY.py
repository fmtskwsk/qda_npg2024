...
class Annealing(Model) :
    ...
    def quantum_annealing(self, BIT_NUM=4, SCALING_FACTOR=50, NUM_READS=50) :
        command_list = ["/usr/bin/python3", "./src/QA_PHY.py", str(BIT_NUM), str(SCALING_FACTOR), str(NUM_READS)]
        subprocess.run(command_list)
    
    def calculate_rmse(self) :
        dx_opt = np.load("./data/dx_opt_PHY.npy")
    ...
    def make_data(self) :
        ...
        np.save("./data/RMSE_PHY", rmse_avg_list)
    ...
    def sensitivity_scaling_factor(self) :
        ...
        np.save("./data/RMSE_SCALING_FACTOR_PHY", rmse_scaling_factor)
        
    def sensitivity_num_reads(self) :
        SAMPLE_NUM = 10
        num_reads_list = [1, 2, 5, 10, 20, 50, 100, 150, 200]
        rmse_num_reads = np.zeros((len(num_reads_list)))
        std_num_reads = np.zeros((len(num_reads_list)))
        time_num_reads = np.zeros((len(num_reads_list)))
        for i, nr in enumerate(num_reads_list) :
            rmse_saple = np.zeros((SAMPLE_NUM))
            time_saple = np.zeros((SAMPLE_NUM))
            for j in range(SAMPLE_NUM) :
                self.quantum_annealing(NUM_READS=nr)
                rmse_avg_list = self.calculate_rmse()
                rmse_saple[j] = rmse_avg_list[0]
                time_saple[j] = np.load("./data/TIME_PHY.npy")
            rmse_num_reads[i] = np.average(rmse_saple)
            std_num_reads[i] = np.std(rmse_saple)
            time_num_reads[i] = np.average(time_saple)
        np.save("./data/RMSE_NUM_READS_PHY", rmse_num_reads)
        np.save("./data/STD_NUM_READS_PHY", std_num_reads)
        np.save("./data/TIME_NUM_READS_PHY", time_num_reads)
