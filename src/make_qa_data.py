...
class VariationalMethod(DataAssimilation) :
    def __init__(self, WINDOW_DAY, b_ii):
        ...
        self.B_MHRHM = np.zeros((self.WINDOW_NUM, self.N, self.N))
        self._2dRHM = np.zeros((self.WINDOW_NUM, self.N))
    ...
    def four_d_var_increment(self) :
        def cost_function(dx_opt) :
            ...
            for i in range(self.WINDOW_DAY_STEP) :
                d = self.d_1L[self.N*i:self.N*(i+1)]
                M_t_0 = self.M_1L_0[self.N*i:self.N*(i+1), :]
        ...
        for w in tqdm(range(self.WINDOW_NUM)) :               
            ...
            self.B_MHRHM[w, :, :] = LA.inv(self.B) + ((self.M_1L_0.T) @ (self.M_1L_0))
            self._2dRHM[w, :] = -2. * ((self.d_1L.T) @ self.M_1L_0)
            optimal_solution = minimize(cost_function, dx_opt, jac=jacobian, method="BFGS")
    ...
    def generate_window_data(self, window_init_index, window_next_index) : 
        ...
        d_1L = np.zeros((self.WINDOW_DAY_STEP*self.N))
